...
import {
  ARQRCodeGenerator,
  PaymentData,
} from "./interaction/ARQRCodeGenerator";
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        data.forEach((obj: DeployedObject) => {
          const coords =
            obj.preciselatitude && obj.preciselongitude
              ? {
                  latitude: obj.preciselatitude,
                  longitude: obj.preciselongitude,
                }
  ...
  const getObjectEmoji = (objectType: string) => {
    switch (objectType) {
      ...
      case "intelligent_assistant":
        return "🧠"; // Brain - AI Intelligence
      case "local_services":
        return "🏪"; // Store - Local Services
      case "payment_terminal":
        return "💳"; // Credit Card - Payment
      case "game_agent":
        return "🎮"; // Game Controller - Gaming
      case "3d_world_builder":
        return "🏗️"; // Construction - 3D Building
      case "home_security":
        return "�"; // Lock - Security
      case "content_creator":
        return "🎨"; // Art Palette - Creative
      case "real_estate_broker":
        return "🏘️"; // Houses - Real Estate
      case "bus_stop_agent":
        return "🚌"; // Bus - Transportation
      // Legacy support
      case "ai_agent":
        return "🤖"; // Robot - AI Agent
      case "tutor":
        return "�"; // Books - Education
      case "landmark":
        return "�"; // Statue - Landmark
      case "building":
                ...
                <a-text
                  value={
                    obj.correctionapplied
                      ? `RTK ±${obj.accuracy?.toFixed(2)}m`
                      : `GPS ±${obj.accuracy?.toFixed(0) || "10"}m`
                  }
                  position="0 -1.0 0"
                  align="center"
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-bold text-green-400">
              {objects[0]?.range_meters?.toFixed(1) || "25.0"}m
            </span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-1">
              {objects.some((obj) => obj.chat_enabled) && (
                <span className="text-blue-400">💬</span>
              )}
              {objects.some((obj) => obj.voice_enabled) && (
                <span className="text-green-400">🎤</span>
              )}
              {objects.some((obj) => obj.mcp_integrations) && (
                <span className="text-green-400">🔗</span>
              )}
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-gray-600">
            <p className="text-xs text-gray-400">
              Use mouse to look around, WASD keys to move. NEAR agents are
              spinning and labeled with their names.
            </p>
          </div>
        </div>
      </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center">
                      <span className="text-lg mr-2">
                        {getObjectEmoji(obj.object_type)}
                      </span>
                  ...
                  <div className="text-xs text-gray-300 space-y-1">
                    ...
                    <div>
                      Accuracy: ±
                      {obj.accuracy?.toFixed(obj.correctionapplied ? 2 : 0) ||
                        "10"}
                      m
                    </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
                <span className="text-2xl mr-3">
                  {getObjectEmoji(selectedObject.object_type)}
                </span>
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <span className="ml-2 text-gray-600">
                  {selectedObject.range_meters?.toFixed(1)}m
                </span>
              </div>
              ...
              <div>
                ...
                <div className="mt-1 font-mono text-xs text-gray-600">
                  <div>
                    Lat:{" "}
                    {(
                      selectedObject.preciselatitude || selectedObject.latitude
                    ).toFixed(8)}
                  </div>
                  <div>
                    Lon:{" "}
                    {(
                      selectedObject.preciselongitude ||
